...
class SeriesListaScreen extends StatelessWidget {
  const SeriesListaScreen({super.key});
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: () => Get.find<SeriesListaController>().filterChanged(),
          ),
        ],
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: () => Get.toNamed('/series/nuevo'),
...
class ListaSeries extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final SeriesListaController controller = Get.put(SeriesListaController());
    return LiquidPullToRefresh(
      onRefresh: () async => controller.pagingController.refresh(),
      child: PagedListView<int, SerieModel>(
        pagingController: controller.pagingController,
        scrollController: ScrollController(),
        padding: const EdgeInsets.all(0),
        builderDelegate: PagedChildBuilderDelegate<SerieModel>(
          itemBuilder: (context, item, index) => ListTileWidget(
            model: ListTileModel(
              title: item.descripcion ?? '',
              subtitle: '${item.serie ?? ''} - ${item.folio ?? ''}',
              text: item.tipo ?? '',
              onTap: () => Get.toNamed('/series/editar', arguments: item),
              trailing: Text(
                item.estatus ?? '',
                style: TextStyle(
                    color:
          ...
          newPageErrorIndicatorBuilder: (context) => const Center(
            child: Text('Error al cargar los series'),
          ),

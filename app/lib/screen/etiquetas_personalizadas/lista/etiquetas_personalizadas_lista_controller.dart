...
class EtiquetasPersonalizadasListaController extends GetxController {
  final UsuarioController usuarioController = Get.find();
  TextEditingController buscarController = TextEditingController();

  static const int _pageSize = 10;
  final PagingController<int, EtiquetasModel> pagingController =
      PagingController(firstPageKey: 0);

  EtiquetasPersonalizadasListaController() {
  ...
  Future<void> fetchItems(int pageKey) async {
    try {
      ...
      final response = await apiService.fetchData('etiquetas/obtener',
          method: Method.POST, body: body);

      final impuestosList = (response['etiquetas'] as List)
          .map((item) => EtiquetasModel.fromJson(item))
          .toList();
